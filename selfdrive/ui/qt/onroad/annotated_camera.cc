...
void AnnotatedCameraWidget::updateState(const UIState &s, const FrogPilotUIState &fs) {
  ...
  if (distance_btn->isEnabled()) {
    distance_btn->move(rightHandDM ? width() - UI_BORDER_SIZE : UI_BORDER_SIZE, frogpilot_nvg->dmIconPosition.y() - distance_btn->height() / 2);
    distance_btn->updateState(s.scene, fs.frogpilot_scene);
  }

  frogpilot_nvg->updateState(fs);
}
...
void AnnotatedCameraWidget::paintEvent(QPaintEvent *event) {
  ...
  // FrogPilot variables
  if (s->scene.world_objects_visible) {
    frogpilot_nvg->paintFrogPilotWidgets(painter, *s, *fs, frogpilot_toggles);
  }
}
