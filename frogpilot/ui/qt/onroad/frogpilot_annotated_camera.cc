...
void FrogPilotAnnotatedCameraWidget::paintFrogPilotWidgets(QPainter &p, UIState &s, FrogPilotUIState &fs, QJsonObject &frogpilot_toggles) {
...
void FrogPilotAnnotatedCameraWidget::paintAdjacentPaths(QPainter &p, const cereal::CarState::Reader &carState, const cereal::ModelDataV2::Reader &model, FrogPilotUIScene &frogpilot_scene, QJsonObject &frogpilot_toggles) {
  ...
  if (frogpilot_scene.lane_width_left != 0 && model.getRoadEdgeStds()[0] < 0.5f) {
    p.save();
  ...
  if (frogpilot_scene.lane_width_right != 0 && model.getRoadEdgeStds()[1] < 0.5f) {
    p.save();
