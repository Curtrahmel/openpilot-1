...
def randomize_distance_icons(available_themes, selected_theme):
  ...
  for name in distance_icons_path.iterdir():
    ...
    theme_path = name
    if not (theme_path / "distance_icons").is_dir():
      continue

    icon_name = name.name
    if icon_name in available_themes and selected_theme not in icon_name:
      continue

    weight = 5 if selected_theme in icon_name else 1
    candidates.extend([icon_name] * weight)
...
def randomize_wheel_image(available_themes, selected_theme):
  ...
  for wheel_file in steering_wheels_path.iterdir():
    ...
    name = wheel_file.stem
    if name in available_themes and selected_theme not in name:
      continue
